...
$(document).ready(function () {
    ...
    $('.menu').click(function (e) {
        ...
        // else if($(this).text()=='다중입력'){
        //     let input = prompt("문자열을 입력하세요", "ex) f(x)=2*x");
        //     let size = {
        //         width: SYMBOL_WIDTH * input.length,
        //         height: SYMBOL_HEIGHT
        //     };
        //     let position = {//randomposition
        //         x: Math.random() * (MathApp.canvas.width - size.width) + size.width / 2,
        //         y: Math.random() * (MathApp.canvas.height - size.height) + size.height / 2
        //     };
        //     let new_symbol = new MathApp.Symbol(position, size, input);
        // }
    })
});